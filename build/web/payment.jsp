...
<html>
    ...
    <body class="dark">
     ...
     String role = (account != null && account.getRole() != null) ? account.getRole() : "";   
        ...
        <div class="container section-infor">
            <div class="header">
                <div>
                    <h2 style="font-size: 18px; font-weight: 600;"><i class="fas fa-map-marker-alt"></i> Địa Chỉ Nhận Hàng</h2>
                    <div class="address">
                        <h3>Nguyễn Văn Kiểm (+84) 336780144</h3>
                        <p>Xóm 3(mới) Nghi Kiều, Xã Nghi Kiều, Huyện Nghi Lộc, Nghệ An</p>
            ...
            <div class="product-list">
                <h3 style="font-size: 18px; font-weight: 600;">Sản phẩm</h3>
                <c:set value="0" var="sum"></c:set>
                <c:forEach items="${requestScope.list}" var="pro">
                    <!-- Cộng dồn tổng số tiền của từng sản phẩm vào biến sum -->
                    <c:set var="sum" value="${sum + (pro.quantity * pro.currentPrice)}"></c:set>

                        <div>
                            <div class="product-item">
                                <span class="badge"> Yêu thích </span>
                                <span style="color: #eee">${pro.sellerName}</span>
                        </div>
                        <div class="product-item">
                            <img alt="${pro.productName}" height="50" src="${pro.imageURL}" width="50" />
                            <div class="details">
                                <h4 class="sub-title" style="font-weight: 600">${pro.productName}</h4>
                                <p>Đổi ý miễn phí 15 ngày</p>
                                <p>Loại: Đen,Cao Cấp (Đẹp ...</p>
                            </div>
                            <div class="price">
                                <fmt:formatNumber value="${pro.currentPrice}" type="number" pattern="#,##0" var="formattedCurrentPrice" />
                                ₫${formattedCurrentPrice.replace(",", ".")}
                            </div>
                            <div class="quantity">${pro.quantity}</div>
                            <div class="total">
                                <fmt:formatNumber value="${pro.quantity * pro.currentPrice}" type="number" pattern="#,##0" var="formattedCurrentPrice" />
                                ₫${formattedCurrentPrice.replace(",", ".")}
                            </div>
                        </div>
                    </div>
        ...
        <div class="container section-voucher">
            ...
            <div class="section payment-method">
                <div>
                    ...
                    ShopCart Xu
                    <span style="color: #999">Không thể sử dụng Xu</span>
                </div>
            </div>
            <hr />
            <div class="section">
                <div style="font-size: 18px; font-weight: 600;" class="section-title">Phương thức thanh toán</div>
            ...
            <div class="section total">
                ...
                <div><fmt:formatNumber value="${sum}" type="number" pattern="#,##0" var="formattedCurrentPrice" />
            ...
            <div class="section total">
                ...
                <div class="amount"><fmt:formatNumber value="${sum+30000}" type="number" pattern="#,##0" var="formattedCurrentPrice" />
                    ₫${formattedCurrentPrice.replace(",", ".")}</div>
            </div>
            <div class="order-button">
                <form id="orderForm"  action="order" method="get" style="display: none;">
                    <input type="hidden" name="list" value="${requestScope.list}">
                    <input type="hidden" name="total" value="${sum+30000}">
                </form>
                <button onclick="submitOrder()">Đặt hàng</button>
            </div>
            <div class="terms">
                Nhấn "Đặt hàng" đồng nghĩa với việc bạn đồng ý tuân theo
                <a href="#">Điều khoản NgKiemShop</a>
            </div>
        </div>
