     ...
     String role = (account != null && account.getRole() != null) ? account.getRole() : "";   
        ...
        <div class="container mt-4">
            <div class="cart-header row">
                <div class="col-1">
                    <input type="checkbox" id="select-all" onchange="updateCartSummary()" />
                </div>
            ...
            <c:forEach items="${requestScope.listCartItem}" var="cart">
                <div class="cart-item row">
                    <div class="col-1">
                        <input type="checkbox" class="product-checkbox" />
                    </div>
                    <div class="col-4">
                        <a href="productdetail?productID=${cart.productID}">
                            <input class="productID" type="hidden" value="${cart.productID}" >
                            <div class="store-name">${cart.sellerName}</div>
                            <div class="row">
                                <div class="col-3">
                                ...
                                <div class="col-9">
                                    ...
                                    <div class="product-details mt-2">Phân Loại Hàng: 13-14in(13.5x31.5cm)</div>

                                    <div class="product-discount">
                                        <input style="display: none" type="text" value="${cart.originalPrice}" name="priceOriginal">
                                        <span class="product-old-price">
                                            <fmt:formatNumber value="${cart.originalPrice}" type="number" pattern="#,##0" var="formattedOriginalPrice" />
                                            ₫${formattedOriginalPrice.replace(",", ".")}
                                        </span>
                                        
                                        <span class="product-price">
                                            <fmt:formatNumber value="${cart.currentPrice}" type="number" pattern="#,##0" var="formattedCurrentPrice" />
                                            ₫${formattedCurrentPrice.replace(",", ".")}
                                        </span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="col-2">
                        <div class="quantity-control">
                            ...
                            <input class="form-control product-quantity" type="number" value="${cart.quantity}" min="1" onchange="updateCartSummary()" />
                            <button class="increase-btn">+</button>
                        </div>
                    </div>
                    <div class="col-2">
                        <c:set var="total" value="${cart.currentPrice * cart.quantity}" />
                        <input type="hidden" name="total1-price" class="total1-price" value="${total}">
                        <div class="total-price1">
                            <fmt:formatNumber value="${total}" type="number" pattern="#,##0" var="formattedCurrentPrice" />
                            ₫${formattedCurrentPrice.replace(",", ".")}
                        </div>
                    </div>
                    <div class="col-1">
                        <button class="remove-link" onclick="deleteCart('${cart.cartItemID}');
                                updateCartSummary()">Xóa</button>
                    </div>
                </div>
            </c:forEach>
            <div class="cart-footer row">
                <div class="col-6">
                    <input type="checkbox" id="select-all-footer" /> Chọn Tất Cả (${requestScope.number})
                    <a class="remove-link" href="#">Xóa</a>
                    <a class="remove-link" href="#">Lưu vào mục Đã thích</a>
                </div>
                <div class="col-6 text-end">
                    <div class="total-price">Tổng thanh toán (0 Sản phẩm): ₫0</div>
                    <form id="checkoutForm" action="payment" method="post" style="display: none;"></form>
                    <button type="button" class="btn checkout-btn" onclick="submitCheckoutForm()">Mua Hàng</button>
                </div>
            </div>
        </div>
            ...
            function submitCheckoutForm() {
                ...
                cartItems.forEach((item) => {
                    ...
                    if (checkbox.checked) {
                        ...
                        const price = item.querySelector(".product-price").textContent.trim().replace("₫", "").replace(".", "");
        ...
        <script type="text/javascript">
            function deleteCart(cartItemID) {
                if (confirm("Bạn có chắc chắn xóa sản phẩm này không?")) {
                    window.location.href = "deleteCart?id=" + cartItemID;
                }
            }
